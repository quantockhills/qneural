...
def main():
    ...
    for i, conv in enumerate(conversions, 1):
        ...
        result = convert_archival_to_new_format(
            conv["old_path"], conv["new_path"], conv["metadata"]
        )

        if result["success"]:
            print("  ✓ Success!")
            print(
                f"  Architecture: {result['architecture_detected']['time_hidden_layers']}x{result['architecture_detected']['time_hidden_units']} (time), "
                f"{result['architecture_detected']['control_hidden_layers']}x{result['architecture_detected']['control_hidden_units']} (control)"
            )
            print(
                f"  Angle range: [{result['angle_range'][0] / 3.14159:.4f}π, {result['angle_range'][1] / 3.14159:.4f}π]"
            )
