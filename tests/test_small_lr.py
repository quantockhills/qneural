"""
Test with smaller LR and longer training
"""
...
optimizer = torch.optim.Adam(network.parameters(), lr=5e-5)  # Smaller LR

time_grid = torch.linspace(0, 1, 101)
inputs = torch.stack([angle.repeat(101), time_grid], dim=1)

print("Training with lr=5e-5...")
best_loss = float('inf')
...
if 1-fidelity < 0.1:
    print("✓ SUCCESS!")
    torch.save(network.state_dict(), 'cz_working_model.pt')
else:
    print(f"Best achieved: {best_loss:.6f}")
