...
class TestTimeOptimalControllerInitialization:
    ...
    def test_network_architectures(self):
        ...
        time_linears = [
            m
            for m in controller.time_predictor.modules()
        ...
        control_linears = [
            m
            for m in controller.control_generator.modules()
            if isinstance(m, torch.nn.Linear)
        ]
...
class TestTimeOptimalControllerForward:
    ...
    def test_different_activations(self):
        ...
        for activation in ["sigmoid", "tanh"]:
            controller = TimeOptimalController(
                time_output_activation=activation, n_time_steps=51
            )
    ...
    def test_detuning_range_scaling(self):
        ...
        controller = TimeOptimalController(
            rabi_max=25.0, detuning_range=(-50.0, 50.0), n_time_steps=101
        )
...
class TestPulseFunctions:
    ...
    def test_detuning_off_resonant_after_gate(self):
        ...
        controller = TimeOptimalController(
            rabi_max=25.0, detuning_range=(-50.0, 50.0), n_time_steps=5
        )
...
class TestTimeOptimalTrainerInitialization:
    ...
    def test_custom_initialization(self):
        ...
        trainer = TimeOptimalTrainer(
            controller, nqubits=2, time_weight=5e-2, time_lr=1e-4, control_lr=1e-3
        )
...
class TestGradientFlow:
    ...
    def test_gradients_flow_through_evolution(self):
        ...
        # Before training
        initial_params = [p.clone() for p in controller.control_generator.parameters()]
        ...
        for initial, current in zip(
            initial_params, controller.control_generator.parameters()
        ):
...
class TestEndToEnd:
    ...
    def test_single_angle_optimization(self):
        ...
        controller = TimeOptimalController(time_bounds=(3.0, 8.5), n_time_steps=51)
        trainer = TimeOptimalTrainer(controller, nqubits=2, time_weight=1e-4)
...
class TestEdgeCases:
    ...
    def test_zero_time_weight(self):
        ...
        controller = TimeOptimalController(n_time_steps=101)
        trainer = TimeOptimalTrainer(controller, nqubits=2, time_weight=0.0)
    ...
    def test_time_bounds_edge_cases(self):
        ...
        t_min, t_max = 5.0, 5.1
        controller = TimeOptimalController(
            time_bounds=(t_min, t_max), rabi_max=rabi_max, n_time_steps=11
        )
    ...
    def test_phase_correction_applied(self):
        ...
        with torch.no_grad():
            ...
            U_corrected = trainer._evolve(
                rabi_fn, detuning_fn, gate_time.item(), apply_corrections=True
            )

            # Without corrections
            U_uncorrected = trainer._evolve(
                rabi_fn, detuning_fn, gate_time.item(), apply_corrections=False
            )
