...
def plot_loss_convergence(
    ...
    ax.scatter(epochs[::max(1, len(epochs)//20)], 
    ...
    ax.text(0.02, 0.98, f'Initial: {losses[0]:.4f}\nFinal: {losses[-1]:.4f}',
...
def plot_training_summary(
    ...
    if n_metrics == 0:
        raise ValueError("History must contain at least one metric (loss, infidelity, or gate_time)")
    ...
    for idx, metric in enumerate(available_metrics):
        ...
        ax.text(0.02, 0.98, f'Initial: {values[0]:.4f}\nFinal: {values[-1]:.4f}',
                transform=ax.transAxes, verticalalignment='top',
