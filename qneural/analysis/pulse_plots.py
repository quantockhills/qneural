...
def plot_pulses_vs_time(
    ...
    for pulse_fn in pulses:
        values = [pulse_fn(t).item() if torch.is_tensor(pulse_fn(t)) else pulse_fn(t) 
                  for t in times]
    ...
    for idx, (ax, values, label) in enumerate(zip(axes, pulse_values, labels)):
        ax.plot(times, values, linewidth=2, color=colors[idx % len(colors)], label=label)
        ...
        # Add statistics
        ax.text(0.02, 0.98, f'Max: {np.max(values):.2f}\nMin: {np.min(values):.2f}',
