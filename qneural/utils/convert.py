...
def detect_architecture(old_network) -> Dict:
    ...
    time_hidden_layers = len(time_linear_layers) - 1  # Total linear layers minus 1 for proper constructor arg
    ...
    control_hidden_layers = len(control_linear_layers) - 1  # Total linear layers minus 1 for proper constructor arg
...
def convert_archival_to_new_format(
    ...
    try:
        ...
        checkpoint = {
            ...
            'history': {'epoch': [], 'loss': [], 'infidelity': [], 'mean_gate_time': []},  # Empty
            'epoch': 0,
...
def load_saved_model(
    ...
    controller.control_generator.load_state_dict(checkpoint['control_network_state_dict'])
    ...
    if print_metadata:
        ...
        for key, value in metadata.items():
            ...
            elif key == 'angle_range_tensor' and isinstance(value, (list, tuple)) and len(value) == 2:
                print(f"  {key}: [{value[0]/np.pi:.4f}π, {value[1]/np.pi:.4f}π]")
            else:
                print(f"  {key}: {value}")
        ...
        print(f"  Total parameters: {sum(p.numel() for p in controller.parameters()):,}")
