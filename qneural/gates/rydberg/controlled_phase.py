...
class ControlledPhaseGate(ABC):
    ...
    def reduce_to_computational_basis(
        self,
    ...
    def compute_phase_correction_matrix(
        self,
        unitary: torch.Tensor
    ) -> torch.Tensor:
        """
...
class ControlledPhaseOptimizer:
    ...
    def generate_pulse(self, angle: float, gate_time: float = None) -> Tuple[List[Callable], float]:
        """
        Generate pulse for specific angle.
