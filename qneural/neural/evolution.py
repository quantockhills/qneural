...
class QuantumEvolver:
    ...
    def evolve(
        ...
        if initial_unitary is None:
            initial_unitary = torch.eye(
                self.full_dim, dtype=torch.cfloat, device=hamiltonian.rabi_ops[0].device
            )

        # Evolve
        evolved_unitary = self._evolve_with_solver(
            hamiltonian, initial_unitary, gate_time
        )
    ...
    def _reduce_to_computational_basis(self, unitary: torch.Tensor) -> torch.Tensor:
        """
    ...
    def _apply_phase_corrections(self, unitary: torch.Tensor) -> torch.Tensor:
        """
    ...
    def compute_infidelity(
        self, achieved_unitary: torch.Tensor, target_unitary: torch.Tensor
    ) -> torch.Tensor:
        """
...
def create_evolver(
    nqubits: int, backend: str = "torchdiffeq", n_time_steps: int = 301, **solver_kwargs
) -> QuantumEvolver:
    """
...
def quick_evolve(
    ...
    return evolver.evolve(
        pulse_functions, gate_time, apply_corrections=apply_corrections
    )
