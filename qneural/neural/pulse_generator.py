...
from ..hardware.rydberg.pulses import piecewise_constant
...
class PhysicalPulseGenerator:
    ...
    def scale_output(self, nn_output: torch.Tensor, control_idx: int) -> torch.Tensor:
        """
    ...
    def generate(
        self, nn_output: torch.Tensor, gate_time: Union[float, torch.Tensor]
    ) -> List[Callable]:
        """
        ...
        if n_times != self.n_time_steps:
            raise ValueError(f"Expected {self.n_time_steps} time steps, got {n_times}")

        if n_controls != self.n_controls:
            raise ValueError(f"Expected {self.n_controls} controls, got {n_controls}")
        ...
        for i in range(self.n_controls):
            ...
            pulse_fn = self._create_pulse_fn(control_values_scaled, gate_time)
    ...
    def _create_pulse_fn(
        self, values: torch.Tensor, gate_time: Union[float, torch.Tensor]
    ) -> Callable:
...
class BatchedPulseGenerator:
    ...
    def generate_batch(
        self, nn_outputs: torch.Tensor, gate_times: torch.Tensor
    ) -> List[List[Callable]]:
        """
...
def create_default_physical_pulse_generator(
    rabi_max: float, detuning_range: Tuple[float, float] = None, n_time_steps: int = 201
